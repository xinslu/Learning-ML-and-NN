...
def cost_function(theta, X, y):
    reg=1
    total_cost = -(1/m)*np.sum(y.T.dot(np.log(probability(theta, X))) + (1-y).T.dot(np.log(1-probability(theta,X))))+(reg/2/m)*np.sum(np.square(theta[1:]))
    return total_cost
...
data = pd.read_csv("./input/ex2data2.txt" , header=None, names=['Microchip 1', 'Microchip 2','Passed'])
